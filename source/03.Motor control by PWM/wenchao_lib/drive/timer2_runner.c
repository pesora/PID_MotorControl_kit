...
void timer2_runner(){
	if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET) //Check whether TIM3 update interrupt occurs
	{
		System_Flag_5Ms = 1;
		TIM_ClearITPendingBit(TIM2, TIM_IT_Update); //Clear the TIMx update interrupt flag
	}
